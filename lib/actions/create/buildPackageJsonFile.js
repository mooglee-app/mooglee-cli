...
module.exports = function ({ appPath, spinner }, next, data) {
  ...
  if (data.appConfig.enableJsonServer === true) {
    ...
    json.scripts['json-server:dev']  = 'NODE_ENV=production node ./json-server/start-json-server';
    json.scripts['json-server:prod'] = 'NODE_ENV=development node ./json-server/start-json-server';
  }
